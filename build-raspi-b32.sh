...
echo -e "${RED}BUILD SCRIPT FOR BUILD MAINLINE KERNEL LINUX RASPBERRY PI 3B 32bits${NC}"
...
sudo make ARCH=arm CROSS_COMPILE=arm-linux-gnueabihf- INSTALL_MOD_PATH=$path/rootfs modules_install
...
./mkimage -A arm -O linux -T script -C none -n ../../build-scripts/pi-tools/bootpi3.scr -d ../../build-scripts/pi-tools/bootpi3.scr ../../build-scripts/pi-tools/boot.scr.uimg
